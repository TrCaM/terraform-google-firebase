...
| gcs\_object\_source | Object containing bucket and name to fetch prompt content from GCS. | <pre>object({<br>    bucket = string<br>    name   = string<br>  })</pre> | `null` | no |
...
| template\_string | Direct text input for the prompt template content. Overrides gcs\_object\_source if provided. | `string` | `null` | no |
